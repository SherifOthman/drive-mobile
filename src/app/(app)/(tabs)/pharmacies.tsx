...
import { PharmacyCard } from "@/src/features/pharmacies/components/pharmacy-card";
...
export default function Pharmacies() {
  ...
  const {
    ...
    hasNextPage,
    isFetchingNextPage,
  } = usePharmacies({ name: debouncedSearch });
      ...
      <View className="flex-1">
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <PharmacyCard
              pharmacy={item}
              onPress={() => router.push(`/(app)/pharmacy/${item.id}` as any)}
              className="mt-4"
            />
          )}
          ListEmptyComponent={
